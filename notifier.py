...
class Notifier:
    ...
    def new_notification(self, watcher_uid):
        if self.config.get_config_watcher(watcher_uid, 'plex_on_or_off') == 1:
            self.logger.py_logger('Notifier', 'Plex is next to be updated now that we had a successful file operation')
            self.plex.update_library(watcher_uid)

        if self.config.get_config_watcher(watcher_uid, 'kodi_on_or_off') == 1:
            self.logger.py_logger('Notifier', 'Kodi is next to be updated now that we had a successful file operation')
            try:
                url = 'http://' + \
                ...
                self.logger.py_logger('Kodi', 'Successful update')
            except requests.exceptions.ConnectionError:
                self.logger.py_logger('Notifier', 'Error in updating Kodi. Could not connect')
